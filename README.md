...
5. *(Optional)* **Run the Next.js front end**
   ...
   The app will start on [http://localhost:3001](http://localhost:3001). You can
   set `PORT=3001` and `NEXT_PUBLIC_API_BASE_URL=http://localhost:3000/api` in
   `frontend/.env.local` to keep the configuration consistent between sessions.
...
- **API helper**: A small wrapper around `fetch` automatically prepends `NEXT_PUBLIC_API_BASE_URL` and injects the bearer token when present.
